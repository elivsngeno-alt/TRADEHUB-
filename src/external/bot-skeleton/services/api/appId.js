    ...
    derivApiPromise = (async () => {
        try {
            ...
            deriv_socket.addEventListener('close', event => {
                console.log('[DerivAPI] WebSocket connection closed:', event?.code, event?.reason || '');
            ...
            deriv_socket.addEventListener('error', event => {
                const message = event && typeof event === 'object' && event.message ? event.message : 'Network connection issue';
                console.warn('[DerivAPI] WebSocket event warning:', message);
                if (derivApiInstance === deriv_api && (deriv_socket.readyState === WebSocket.CLOSED || deriv_socket.readyState === WebSocket.CLOSING)) {
                    derivApiInstance = null;
                    currentWebSocketURL = null;
                }
            });
