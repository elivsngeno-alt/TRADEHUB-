...
export default class TicksService {
    ...
    requestTicks(options) {
        ...
        return new Promise(resolve => {
            ...
            doUntilDone(() => api_base.api.send(request_object), ['AlreadySubscribed', 'InvalidSymbol'], api_base)
                .then(r => {
                    if (r?.error) {
                        console.warn('[TicksService] ticks_history response error:', r.error);
                        resolve([]);
                        return;
                    }
                    if (style === 'ticks') {
                ...
                .catch(error => {
                    // Handle AlreadySubscribed and InvalidSymbol errors gracefully - they're not fatal
                    ...
                    console.warn(`[TicksService] ticks_history request failed for symbol '${symbol}':`, error?.error?.message || error?.message || error);
                    resolve([]);
                });
        });
    }
