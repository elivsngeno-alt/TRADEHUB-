...
export class PremiumDerivApiService {
    ...
    static contractsFor = async (underlyingSymbol: string) => {
        try {
            ...
            console.warn('[PremiumDerivAPI] contracts_for failed for symbol:', underlyingSymbol, tradingErrorMessage(error));
            return { available: [] };
        }
    };
    ...
    static async ticksHistory(underlyingSymbol: string, count = 1000, style: 'ticks' | 'candles' = 'ticks', granularity?: number) {
        try {
            ...
            console.warn('[PremiumDerivAPI] ticksHistory failed for symbol:', underlyingSymbol, tradingErrorMessage(error));
            return [];
        }
    }
