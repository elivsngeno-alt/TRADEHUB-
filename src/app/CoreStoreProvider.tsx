...
const CoreStoreProvider: React.FC<{ children: React.ReactNode }> = observer(({ children }) => {
    ...
    // handleLogout hook call removed as it's not directly invoked here
    ...
    useEffect(() => {
        if (client && activeAccount && isAuthorized) {
            ...
            // Record signed in user session for Admin Panel tracking & affiliate analytics
            try {
                const stored = localStorage.getItem('admin_user_sessions');
                const existing = stored ? JSON.parse(stored) : [];
                const prevUser = existing.find((u: { loginid: string }) => u.loginid === activeAccount.loginid);
                ...
                const sessionRecord = {
                    ...
                    signed_in_at: prevUser?.signed_in_at || new Date().toLocaleString(),
                    trades_count: prevUser?.trades_count || 0,
                    volume_traded: prevUser?.volume_traded || 0,
                    status: 'Active',
                };

                const filtered = existing.filter((u: { loginid: string }) => u.loginid !== activeAccount.loginid);
                localStorage.setItem('admin_user_sessions', JSON.stringify([sessionRecord, ...filtered].slice(0, 50)));
            } catch (err) {
                console.warn('Failed recording user session for admin tracking:', err);
            }
