...
export const getSocketURL = async (): Promise<string> => {
    ...
    try {
        ...
        console.warn('[DerivWS] Authenticated WebSocket setup failed, falling back to public endpoint:', message);
        return getDefaultServerURL();
    }
};
...
export const generateOAuthURL = async (prompt?: string) => {
    try {
        ...
        // Deriv's current OAuth authorization endpoint is /oauth2/auth.
        // Using /authorize returns Deriv's "We couldn't find that page" screen.
