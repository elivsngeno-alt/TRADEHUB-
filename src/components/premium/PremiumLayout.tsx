...
const PremiumLayout = observer(() => {
    ...
    const [guestSessionActive, setGuestSessionActive] = useState(() => localStorage.getItem('guest_session_active') === 'true');
    ...
    if (section === 'admin') {
        return (
            <div style={{ minHeight: '100vh', background: '#0f172a', color: '#f8fafc' }}>
                <AdminPanelPage onExit={() => changeSection('dashboard')} />
            </div>
        );
    }
    ...
    >
        ...
        <AIAssistant openBotBuilder={openBotBuilder} />
