...
export const DEFAULT_NAVIGATION = NAVIGATION_CATALOG.filter(item => item.id !== 'admin').map(item => item.id);
...
const CATALOG_IDS = new Set(NAVIGATION_CATALOG.map(item => item.id));
...
const normalizeNavigation = (value: unknown): PremiumSection[] => {
    ...
    if (!Array.isArray(value)) {
        const base = DEFAULT_NAVIGATION.filter(item => item !== 'admin' || isAdminAuth);
        if (isAdminAuth && !base.includes('admin')) base.push('admin');
        return base;
    }
    ...
    const navigation = value
        ...
        .filter(item => CATALOG_IDS.has(item) && (item !== 'admin' || isAdminAuth) && !seen.has(item) && Boolean(seen.add(item)));

    if (isAdminAuth && !navigation.includes('admin')) {
...
export const useSiteCustomization = (): SiteCustomization => {
    ...
    const syncCustomization = (remotePayload?: any) => {
        let localConfig: any = null;
        try {
            const stored = localStorage.getItem('admin_panel_config');
            if (stored) localConfig = JSON.parse(stored);
        } catch (e) {
            console.error('Failed parsing admin_panel_config', e);
        }

        const rawNav = localConfig?.navigation ?? remotePayload?.navigation;
        const rawColors = localConfig?.colors ?? remotePayload?.colors;

        setState({
            navigation: normalizeNavigation(rawNav),
    ...
    useEffect(() => {
        ...
        void readJson([
            ...
            if (!alive) return;
            syncCustomization(payload);
        });

        const handleCustomEvent = () => syncCustomization();
        window.addEventListener('admin_config_updated', handleCustomEvent);
        window.addEventListener('storage', handleCustomEvent);

        return () => {
            alive = false;
            window.removeEventListener('admin_config_updated', handleCustomEvent);
