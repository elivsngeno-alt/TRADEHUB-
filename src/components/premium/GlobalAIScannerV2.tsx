...
import { analyzeMarketWithGemini } from '@/services/gemini-scanner.service';
...
const analyze = async (market: Market, strategy: Strategy, requestedTicks: number): Promise<Result> => {
    ...
    const geminiRes = await analyzeMarketWithGemini({
        symbol: market.symbol,
        name: market.name,
        pipSize: market.pipSize,
        digits,
        strategyOver: strategy.over,
        strategyUnder: strategy.under,
    });

    const recent = digits.slice(-50);
    const primaryRate = rate(digits, geminiRes.recommendedSide, geminiRes.primaryBarrier);
    const recentRate = rate(recent, geminiRes.recommendedSide, geminiRes.primaryBarrier);

    return {
        symbol: market.symbol,
        market: market.name,
        sampleSize: digits.length,
        primarySide: geminiRes.recommendedSide,
        primaryBarrier: geminiRes.primaryBarrier,
        recoverySide: geminiRes.recoverySide,
        recoveryBarrier: geminiRes.recoveryBarrier,
        confidence: geminiRes.aiConfidence,
        primaryRate,
        recentRate,
        aiInsight: geminiRes.aiInsight,
        isGeminiPowered: geminiRes.isGeminiPowered,
    };
};
...
const GlobalAIScannerV2 = ({ openBotBuilder }: { openBotBuilder: () => void }) => {
    ...
    const loadBot = useCallback(async () => {
        ...
        try {
            ...
            await loadStrategyToBotBuilder(
                configured,
                `AI ${result.symbol} ${textFor(result.primarySide, result.primaryBarrier)}.xml`,
                openBotBuilder
            );
    ...
    >
        <header className='prodb-ai2-panel__header' onPointerDown={panelDown} onPointerMove={panelMove} onPointerUp={panelUp}>
            <strong>✨ Gemini AI Entry Scanner</strong><span>Drag window</span><button type='button' onClick={() => setOpen(false)} aria-label='Close scanner'>×</button>
        </header>
        <div className='prodb-ai2-panel__body'>
            ...
            <div className='prodb-ai2-intro'>
                <div>
                    <strong>Gemini 3.8 Flash Engine</strong>
                    <small>Scans synthetic index tick distributions and predicts optimal Over/Under barriers using Gemini AI.</small>
                </div>
                <label><span>TICKS</span><input type='number' min='100' max='5000' step='100' value={tickCount} onChange={e => setTickCount(Number(e.target.value))} /></label>
            </div>
            <div className='prodb-ai2-selection'>
                <div>
                    <span>SELECTED MARKET</span>
                    <strong>{result?.market || 'Scan to find the best market'}</strong>
                    <small>{result ? `${result.symbol} · ${result.sampleSize} ticks` : '—'}</small>
                </div>
                <div>
                    <span>AI TRADE SIGNAL</span>
                    <strong>{result ? textFor(result.primarySide, result.primaryBarrier) : 'Waiting for scan'}</strong>
                    <small>{result ? `Recovery: ${textFor(result.recoverySide, result.recoveryBarrier)}` : '—'}</small>
                </div>
            </div>

            {result?.aiInsight && (
                <div style={{ background: 'rgba(16, 185, 129, 0.08)', border: '1px solid rgba(16, 185, 129, 0.25)', borderRadius: 8, padding: '10px 12px', fontSize: 12, color: 'var(--site-text, #e2e8f0)', marginTop: 4 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, fontWeight: 700, color: '#10b981' }}>
                        <span>🤖 Gemini AI Pattern Insight</span>
                        <span>Confidence: {(result.confidence * 100).toFixed(1)}%</span>
                    </div>
                    <div style={{ opacity: 0.9, lineHeight: 1.4 }}>{result.aiInsight}</div>
                </div>
            )}
            ...
            <div className='prodb-ai2-actions'><button type='button' className='is-primary' disabled={scanning || loading} onClick={() => void scan()}>{scanning ? 'Scanning with Gemini…' : 'Scan Markets with AI'}</button><button type='button' disabled={!result || scanning || loading} onClick={() => void loadBot()}>{loading ? 'Configuring…' : 'Load AI Signal Bot'}</button></div>
        </div>
    </section> : null;
