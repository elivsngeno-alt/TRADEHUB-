...
const SpeedBotPage = () => {
    ...
    const [executionSpeed, setExecutionSpeed] = useState<'normal' | 'faster'>(() => {
        return (localStorage.getItem('prodb.speed-bot.execution-speed') as 'normal' | 'faster') || 'faster';
    });
    const [running, setRunning] = useState(false);
    const speedRef = useRef<'normal' | 'faster'>(executionSpeed);

    useEffect(() => {
        speedRef.current = executionSpeed;
        localStorage.setItem('prodb.speed-bot.execution-speed', executionSpeed);
    }, [executionSpeed]);
    ...
    return <div className='prodb-speed-page'>
        ...
        <section className='prodb-speed-card'>
            ...
            <div className='prodb-speed-summary'>
                <b>Current tick: {currentTick?.quote ?? '—'}</b>
                <span>{selectedContract.label}{selectedContract.needsPrediction ? ` ${settings.prediction}` : ''} · {settings.duration}{settings.durationUnit} · {executionSpeed === 'faster' ? '⚡ Instant High-Speed Execution' : '🐢 Normal Cadence Execution'}</span>
            </div>

            {/* Execution Speed Selector Mode */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, margin: '12px 0 16px', background: 'var(--site-card-subtle-bg, #f1f5f9)', padding: '8px 14px', borderRadius: 10, border: '1px solid var(--site-border, #cbd5e1)' }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--site-text, #0f172a)' }}>Bot Execution Speed:</span>
                <button
                    type='button'
                    disabled={running}
                    style={{
                        ...
                        background: executionSpeed === 'faster' ? '#059669' : 'transparent',
                        color: executionSpeed === 'faster' ? '#ffffff' : '#059669',
                        fontWeight: 700,
                        fontSize: 13,
                    ...
                    onClick={() => setExecutionSpeed('faster')}
                >
                    ⚡ Faster / Instant Turbo
                </button>
                <button
                    type='button'
                    disabled={running}
                    style={{
                        ...
                        background: executionSpeed === 'normal' ? '#64748b' : 'transparent',
                        color: executionSpeed === 'normal' ? '#ffffff' : '#64748b',
                        fontWeight: 700,
                        fontSize: 13,
                        cursor: running ? 'not-allowed' : 'pointer',
                        display: 'flex',
