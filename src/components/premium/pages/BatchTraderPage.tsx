...
const BatchTraderPage = () => {
    ...
    const [executionSpeed, setExecutionSpeed] = useState<'normal' | 'faster'>('faster');
    ...
    useEffect(() => {
        PremiumDerivApiService.activeSymbols().then(items => {
            ...
            setMarkets(list);
            const preferred = list.find(item => item.symbol === '1HZ100V') || list.find(item => item.symbol === 'R_100') || list[0];
    ...
    return <div className='prodb-batch-page'>
        <section className='prodb-batch-card'>
            ...
            <div className={`prodb-batch-summary ${armed ? 'is-armed' : ''}`}>
                ...
                <span>Total stake / tick: {totalBatchStake.toFixed(2)} {currency} · {executionSpeed === 'faster' ? '⚡ Faster / Parallel Async' : '🐢 Normal Cadence'}</span>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, margin: '12px 0 16px', background: 'var(--site-card-subtle-bg, #f1f5f9)', padding: '8px 14px', borderRadius: 10, border: '1px solid var(--site-border, #cbd5e1)' }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--site-text, #0f172a)' }}>Batch Execution Speed:</span>
                <button
                    type='button'
                    disabled={armed}
                    style={{
